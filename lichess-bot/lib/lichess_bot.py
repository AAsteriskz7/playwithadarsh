...
def write_pgn_records(pgn_queue: PGN_QUEUE_TYPE, config: Configuration, username: str) -> None:
    ...
    while True:
        ...
        try:
            event = pgn_queue.get()
            mark_task_done = True
            logger.debug(f"Received event: {event}")  # Add logging
...
def get_headers(game: model.Game) -> dict[str, Union[str, int]]:
    ...
    logger.debug(f"Headers created: {headers}")
    return headers
...
def save_pgn_record(event: EventType, config: Configuration, user_name: str) -> None:
    ...
    """
    if event is None or event["game"] is None:
        logger.error("Error: event or event['game'] is None")
        return
    pgn = event["game"]["pgn"]
    pgn_headers = chess.pgn.read_headers(io.StringIO(pgn))
    if not config.pgn_directory or pgn_headers is None:
        logger.error("PGN directory is not set or PGN headers are None")
        return
    ...
    logger.debug(f"PGN record saved for game ID: {game_id}")
